@testset "yen" begin
    ...
    @testset "Types" begin
        ...
        for T in type_list
            img = T.(img_gray)
            @test_reference "References/Yen_Gray.png" Gray.(binarize(img, f)) by=binarization_equality()
        end
        ...
        for T in type_list
            img = T.(img_gray)
            @test_reference "References/Yen_Color3.png" Gray.(binarize(img, f)) by=binarization_equality()
        end
    end
