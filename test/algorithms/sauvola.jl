@testset "sauvola" begin
    ...
    @testset "Types" begin
        ...
        for T in type_list
            img = T.(img_gray)
            @test_reference "References/Sauvola_Gray.png" Gray.(binarize(img, f)) by=binarization_equality()
        end
        ...
        for T in type_list
            img = T.(img_gray)
            @test_reference "References/Sauvola_Color3.png" Gray.(binarize(img, f)) by=binarization_equality()
        end
    end
