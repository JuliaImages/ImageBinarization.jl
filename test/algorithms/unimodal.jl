@testset "unimodal" begin
    ...
    @testset "Types" begin
        # although Unimodal doesn't perform well on lena image,
        # we need it to test the Color3 support.
        ...
        for T in type_list
            img = T.(img_gray)
            @test_reference "References/UnimodalRosin_Gray.png" Gray.(binarize(img, f)) by=binarization_equality()
        end
        ...
        for T in type_list
            img = T.(img_gray)
            @test_reference "References/UnimodalRosin_Color3.png" Gray.(binarize(img, f)) by=binarization_equality()
        end
    end
